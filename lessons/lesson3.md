...
For the rest of this module, we'll be working with `demo.sav`. If you don't already have it open, take a moment now to open it.
...
When creating variables, its type is one of the most important things to define. There are several options offered, but you'll usually be working with the Numeric and String types.

<img src="../assets/img/lessons/data2.png" alt="" width="70%" style="border: solid 2px black">

Numeric types allow for numerical input. This can be used to represent both numerical values, but also categorical data. 

When creating numeric types, you can specify how many decimal places to use. For example, any categorical data in our datasheet has 0 decimal places because we only want to be working with whole numbers.

Strings allow for character input. In this example datasheet, only `gender` uses String. However, we could use strings to store participants' names.
...
As discussed in [lesson1](lesson1), variables have a name and a descriptive label. 
...
Data labels are used to map a data value to a string label. In the example below, the numbers 1 through 5 are mapped to levels of satisfaction.
...
You can assign values to represent missing values or data that should be ignored in analyses.  
...
Finally, we'll talk about a variable's measure type. Numeric data can have 3 different measure types, and it's important to understand the differences between them.

Scale
: Data that can be shown on a measurement scale. TODO

Ordinal
: Categorized data with an order/ranking.
...
<img src="../assets/img/lessons/data6.png" alt="" width="100%" style="border: solid 2px black">

The only important thing to mention here is that you're able to toggle between displaying raw data and labeled data using the `Value Labels` button at the top.
